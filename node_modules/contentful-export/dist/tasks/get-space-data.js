...
function getFullSourceSpace({
  ...
  return new _listr2.default([{
    ...
    task: (0, _listr3.wrapTask)((ctx, task) => {
      return pagedGet({ source: ctx.environment, method: 'getEntries', query: queryEntries }).then(extractItems).then(items => filterDrafts(items, includeDrafts)).then(items => {
    ...
    task: (0, _listr3.wrapTask)((ctx, task) => {
      return pagedGet({ source: ctx.environment, method: 'getAssets', query: queryAssets }).then(extractItems).then(items => filterDrafts(items, includeDrafts)).then(items => {
