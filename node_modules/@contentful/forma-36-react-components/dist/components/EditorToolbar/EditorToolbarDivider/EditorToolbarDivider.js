"use strict";

var _interopRequireWildcard = require("@babel/runtime/helpers/interopRequireWildcard");

var _interopRequireDefault = require("@babel/runtime/helpers/interopRequireDefault");

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = exports.EditorToolbarDivider = void 0;

var _extends2 = _interopRequireDefault(require("@babel/runtime/helpers/extends"));

var _objectWithoutProperties2 = _interopRequireDefault(require("@babel/runtime/helpers/objectWithoutProperties"));

var _classCallCheck2 = _interopRequireDefault(require("@babel/runtime/helpers/classCallCheck"));

var _createClass2 = _interopRequireDefault(require("@babel/runtime/helpers/createClass"));

var _possibleConstructorReturn2 = _interopRequireDefault(require("@babel/runtime/helpers/possibleConstructorReturn"));

var _getPrototypeOf2 = _interopRequireDefault(require("@babel/runtime/helpers/getPrototypeOf"));

var _inherits2 = _interopRequireDefault(require("@babel/runtime/helpers/inherits"));

var _defineProperty2 = _interopRequireDefault(require("@babel/runtime/helpers/defineProperty"));

var _propTypes = _interopRequireDefault(require("prop-types"));

var _react = _interopRequireWildcard(require("react"));

var _classnames = _interopRequireDefault(require("classnames"));

var styles = {
  "EditorToolbarDivider": "EditorToolbarDivider__EditorToolbarDivider___22NLC"
};

var EditorToolbarDivider =
/*#__PURE__*/
function (_Component) {
  (0, _inherits2.default)(EditorToolbarDivider, _Component);

  function EditorToolbarDivider() {
    (0, _classCallCheck2.default)(this, EditorToolbarDivider);
    return (0, _possibleConstructorReturn2.default)(this, (0, _getPrototypeOf2.default)(EditorToolbarDivider).apply(this, arguments));
  }

  (0, _createClass2.default)(EditorToolbarDivider, [{
    key: "render",
    value: function render() {
      var _this$props = this.props,
          extraClassNames = _this$props.extraClassNames,
          testId = _this$props.testId,
          otherProps = (0, _objectWithoutProperties2.default)(_this$props, ["extraClassNames", "testId"]);
      var classNames = (0, _classnames.default)(styles['EditorToolbarDivider'], extraClassNames);
      return _react.default.createElement("span", (0, _extends2.default)({
        "data-test-id": testId,
        className: classNames
      }, otherProps));
    }
  }]);
  return EditorToolbarDivider;
}(_react.Component);

exports.EditorToolbarDivider = EditorToolbarDivider;
EditorToolbarDivider.displayName = "EditorToolbarDivider";
(0, _defineProperty2.default)(EditorToolbarDivider, "propTypes", {
  testId: _propTypes.default.string,
  extraClassNames: _propTypes.default.string
});
(0, _defineProperty2.default)(EditorToolbarDivider, "defaultProps", {
  testId: 'cf-editor-toolbar-divider'
});
var _default = EditorToolbarDivider;
exports.default = _default;