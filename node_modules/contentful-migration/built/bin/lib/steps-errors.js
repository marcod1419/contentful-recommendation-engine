...
const renderStepsErrors = function (errors) {
    ...
    for (const file of Object.keys(errorsByFile)) {
        ...
        const fileErrorsMessage = chalk_1.default `{red Errors in ${file}}\n\n`;
        const errorMessages = errorsByFile[file].map((error) => {
            ...
            const beforeLines = before.map((line) => chalk_1.default `${line}\n`).join('');
            const afterLines = after.map((line) => chalk_1.default `${line}\n`).join('');
            const highlightedLine = chalk_1.default `{bold ${line}}\n`;
            const formattedCode = beforeLines + highlightedLine + afterLines;
            return chalk_1.default `{red Line ${String(callsite.line)}:} {bold ${error.message}}\n${formattedCode}`;
        }).join('\n');
        messages.push(`${fileErrorsMessage}${errorMessages}`);
    }
    console.error(chalk_1.default `{red.bold Validation failed}\n\n`);
