...
const migration = exports.migration = async argv => {
  await (0, _assertions.assertLoggedIn)({ managementToken: argv.managementToken, paramName: '--management-token' });
  await (0, _assertions.assertSpaceIdProvided)(argv);

  const { cmaToken, activeSpaceId, proxy, rawProxy } = await (0, _context.getContext)();
  ...
  const options = _extends({}, argv, { spaceId, managementApplication, managementFeature, managementToken });
  if (proxy) {
