...
const getEnvironment = async function (cmaToken, spaceId, environmentId) {
  const client = await createManagementClient(cmaToken);
  const space = await client.getSpace(spaceId);
  return space.getEnvironment(environmentId);
};

async function generateMigration(argv) {
  ...
  const environment = await getEnvironment(managementToken, spaceId, environmentId);
