...
const listHandler = exports.listHandler = async argv => {
  const ctx = await (0, _context.getContext)();
  const configList = Object.keys(ctx).map(key => {
    if (key === 'proxy') {
      return `${_emojic2.default.gear}  ${key}: ${(0, _proxy.proxyObjectToString)(ctx[key])}`;
    } else {
      return `${_emojic2.default.gear}  ${key}: ${ctx[key]}`;
    }
  });
